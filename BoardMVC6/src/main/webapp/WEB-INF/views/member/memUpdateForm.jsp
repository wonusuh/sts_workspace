	...
	<div class="container">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<form name="updateForm" action="${cp}/member/memUpdate.do"
					method="post">
					<input type="hidden" name="${_csrf.parameterName}"
						value="${_csrf.token}" /> <input id="memID" name="memID"
						value="${mvo.memID}" readonly="readonly" /> <input type="hidden"
						id="memPassword" name="memPassword" value="" />
					<table class="table table-bordered"
								...
								<div class="form-group"
									...
									<div class="btn-group" data-toggle="buttons">
										<label
											...
											<input type="radio" name="memGender" autocomplete="off"
											value="남자"
						...
						<tr>
							...
							<td colspan="2"><input type="checkbox"
								name="authList[0].auth" value="ROLE_USER"
								<c:forEach var="authVO" items="${mvo.authList}">
                    <c:if test="${authVO.auth eq 'ROLE_USER'}">
                      checked
                    </c:if> 
                  </c:forEach> />
								ROLE_USER <input type="checkbox" name="authList[1].auth"
								value="ROLE_MANAGER"
								<c:forEach var="authVO" items="${mvo.authList}">
                    <c:if test="${authVO.auth eq 'ROLE_MANAGER'}">
                      checked
								...
								ROLE_MANAGER <input type="checkbox" name="authList[2].auth"
								value="ROLE_ADMIN"
								<c:forEach var="authVO" items="${mvo.authList}">
                      ...
                      checked
						...
						<tr>
							<td colspan="3" style="text-align: left;"><span
								id="passMessage" style="color: red"></span><input type="submit"
								class="btn btn-primary btn-sm pull-right" value="수정"
								onclick="location.href='${cp}/member/memUpdate.do'" /></td>
						</tr>
					</table>
				</form>
			</div>
